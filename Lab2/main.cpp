...
int main() {
    ...
    while (window.isOpen()) {
        ...
        drawGraph(window, [](float x) { return -x; }, -10, 10, 200, 200, sf::Color::Blue);

        // 5 _ Отрисовка графика y2 = x + 9 (заменяем на вертикальную x = -3)
        {
            float xConst = -3.0f;
            float screenX = 400 + xConst * 25.0f;  // Было 10.0f, стало 20.0f (увеличение в 2 раза)
            float screenY1 = 300 - (-10.0f) * 25.0f;
            float screenY2 = 300 - (10.0f) * 25.0f;
